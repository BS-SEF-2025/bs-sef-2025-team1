...
import { AddUser, User } from "./schema.js";
...
export class UserDal {
  ...
  constructor(
    ...
    private collectionName: string = usersCollectionName,
  ) {
  ...
  findOrCreate = async (
    ...
    return await this.addUserGoogle(userData);
  };

  addUserGoogle = async (userData: DecodedIdToken): Promise<User> => {
  ...
  addUser = async (userData: AddUser): Promise<User> => {
    // Check if user already exists
    const existingUser = await this.getUserByEmail(userData.email);
    if (existingUser) {
      throw new Error("User with this email already exists");
